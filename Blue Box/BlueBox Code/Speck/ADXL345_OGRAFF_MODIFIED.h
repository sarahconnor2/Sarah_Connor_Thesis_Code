...
public:
 ...
 // bool getInterruptSource(byte interruptBit);
  ...
  //I added these. Prav 18/6
  void setFIFOMode(uint8_t FIFOMode);
  uint8_t getFIFOMode();
  uint8_t getFIFOSampleSize();
  bool setFIFOSampleSize(uint8_t FIFOSampleSize);
  uint8_t getIntWatermarkSource();
  uint16_t getX();
  uint16_t getY();
  uint16_t getZ();
  uint8_t getNumFIFOEntries();
  //SPISettings settingsADXL(4000000,MSBFIRST,SPI_MODE3);
