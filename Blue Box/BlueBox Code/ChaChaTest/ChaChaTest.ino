...
int16_t ax, ay, az;
byte mac[6];
byte dataPacket[6 + 8 + 4 + 96 + 4 + 4 + 1];
byte enc_dataPacket[139];
...
byte iv[8] = {0x2e, 0xec, 0x64, 0x25, 0xb3, 0xec, 0xf0, 0x63};
...
bool encryptData(ChaCha *cipher)
{
    size_t posn, len;
    size_t inc = 128;
...
bool decryptData(ChaCha *cipher)
{
    size_t posn, len;
    size_t inc = 128;
    ...
    memset(decrypted, 0xBA, sizeof(decrypted));

    for (posn = 0; posn < size; posn += inc) {
        len = size - posn;
        if (len > inc)
            len = inc;
        cipher->decrypt(decrypted + posn, enc_dataPacket + posn, len);
    }

    if (memcmp(decrypted, dataPacket, size) != 0)
    {
